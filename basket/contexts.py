...
from django.shortcuts import get_object_or_404  # Used to retrieve a product or raise
...
def basket_contents(request):
    ...
    basket_items = []  # Will hold the list of items in the basket with details
                        # (e.g., product, quantity, subtotal)
    total = 0  # Holds the total value of the basket (before delivery costs)
    basket = request.session.get('basket', {})  # Retrieve the basket from session
    ...
    for item_id, quantity in basket.items():
        if quantity is not None:  # Ensure there's a valid quantity (not None)
            product = get_object_or_404(Print, pk=item_id)  # Get the product using
                                                           # its ID (raise 404 if not found)
            ...
            basket_items.append({
                ...
                'quantity': quantity,
                'subtotal': subtotal,
                'product': product,  # Add the product object to the context for use
                                      # in templates
            })
    ...
    context = {
        ...
        'free_delivery_delta': free_delivery_delta,  # How much more the user needs
                                                     # to spend for free delivery
        'free_delivery_threshold': settings.FREE_DELIVERY_THRESHOLD,  # The value threshold
                                                                      # for free delivery
