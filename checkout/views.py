...
def cache_checkout_data(request):
    ...
    try:
        # Get the PaymentIntent ID from the client secret (needed for modifying
        # the payment intent)
        ...
        # Modify the PaymentIntent to attach additional metadata (basket info,
        # user info, etc.)
        stripe.PaymentIntent.modify(pid, metadata={
            'basket': json.dumps(request.session.get('basket', {})),  # Serialize
            'save_info': request.POST.get('save_info'),  # Whether the user
        ...
        # If any error occurs, show an error message and return a 400 HTTP response
...
def checkout(request):
    ...
    stripe_public_key = settings.STRIPE_PUBLIC_KEY  # Get the Stripe public key
    stripe_secret_key = settings.STRIPE_SECRET_KEY  # Get the Stripe secret key

    if request.method == 'POST':  # Handle form submission when the user submits
        # their checkout details
        ...
        if order_form.is_valid():
            ...
            for item_id, item_data in basket.items():
                try:
                    product = Print.objects.get(id=item_id)  # Retrieve the product
                    # from the database
        ...
        stripe_total = round(total * 100)  # Stripe expects the total in cents
        ...
        if request.user.is_authenticated:
            try:
                ...
                order_form = OrderForm()  # If the profile doesn't exist, show a blank form
        else:
    ...
    context = {
        ...
        'client_secret': intent.client_secret,  # Send the client secret for Stripe
        # on the frontend
    }
...
def checkout_success(request, order_number):
    ...
    This view is triggered after a successful payment, and it displays a success 
    message and order details.
    """
    ...
    if save_info:  # If the user opted to save their info, update their profile
    ...
    messages.success(request, f'Order successfully processed! Your order number '
                             f'is {order_number}. A confirmation email will be sent '
                             f'to {order.email}.')
    ...
    # Set the context for rendering the checkout success page
    template = 'checkout/checkout_success.html'
    context = {
        'order': order,
    }

    return render(request, template, context)  # Render the success page
